...
void testModel(const boost::filesystem::path &path, std::vector<vfh_model> &models,
  ...
  pcl::console::print_info("object %s  match %s a distance of: %f\n", 
    path.filename().c_str(), models.at(k_indices[0][0]).first.c_str (), k_distances[0][0]);
...
void match(const boost::filesystem::path &base_dir, const std::string &extension, std::vector<xyz_model> &templates,
               const flann::Index<flann::L2_Simple<float> >& index, float radius, flann::Matrix<int>& indices, int& nums) {
  ...
  for (boost::filesystem::directory_iterator it(base_dir); it != boost::filesystem::directory_iterator (); ++it)
  {
    ...
    indices = flann::Matrix<int>(new int[max_num], 1, max_num);
    ...
    nums = index.radiusSearch(p, indices, distances, radius, flann::SearchParams (512));
    delete []p.ptr();
    // for (size_t i = 0; i < nums; i++) {
    //   pcl::console::print_info ("object %s  nearby templates %s a distance of: %f\n", 
    //     it->path().filename().c_str(), templates.at (indices[0][i]).first.c_str (), distances[0][i]);   
    // }
    if (nums == 0) {
    ...
    loadFeatureModels(templates, indices, nums, models);
    ...
    int k = nums;
    double thresh = 30;
    testModel(base_dir / it->path ().filename (), models, model_index, k, thresh);
